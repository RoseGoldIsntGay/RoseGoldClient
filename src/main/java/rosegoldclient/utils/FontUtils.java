...
public class FontUtils {
    ...
    public static void drawString(String text, int x, int y, boolean shadow) {
        String[] lines = text.split("\n");
        for(String line : lines) {
            Main.mc.fontRenderer.drawString(line, x, y, Color.WHITE.getRGB(), shadow);
            y += getLineHeight() + 1;
        }
    }
    ...
    public static int getStringWidth(String text) {
        ...
        int longestLine = 0;
        for(String line : lines) {
            int lineWidth = Main.mc.fontRenderer.getStringWidth(line);
    ...
    public static int getLineHeight() {
        return Main.mc.fontRenderer.FONT_HEIGHT;
    }
